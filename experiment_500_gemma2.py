"""
=============================================================
DeceptionProbe — 500-Scenario Experiment on Gemma-2-9B-IT
=============================================================
CROSS-ARCHITECTURE GENERALIZATION TEST — 4th Architecture (Google)
...
This experiment: google/gemma-2-9b-it (Google, 9B, Sliding+Local Attention) → ???

Purpose: Prove that the deception signal is UNIVERSAL across:
  - FOUR completely different model architectures
  - FOUR different training labs (Alibaba, Mistral AI, Meta, Google)

Model: google/gemma-2-9b-it (9B, float16)
GPU:   A100 (80GB VRAM) via Colab
Layer: 21 (middle of 42 layers)
Hidden dim: 3584

435 diverse scenarios across 16 categories (870 samples)
with 5 independent length confound controls.

Expected runtime on A100: ~40 minutes
=============================================================
"""
...
SAVE_DIR = Path("/content/results_gemma2")
SAVE_DIR.mkdir(parents=True, exist_ok=True)

MODEL_NAME = "google/gemma-2-9b-it"
TARGET_LAYER = 21  # Middle of 42 layers
TRAIN_RATIO = 0.8
...
def clear_hf_cache(model_pattern="gemma"):
...
# ============================================================
# LOAD MODEL — Gemma-2-9B-IT (Google, float16)
# ============================================================
...
print(f"Clearing HF cache to free disk space...")
monitor_resources("AFTER MODEL LOAD (before cache clear)")
clear_hf_cache("gemma")
...
print(f"{'='*95}")
print(f"")
print(f"{'Metric':<35s} {'Qwen-3B':>12s} {'Mistral-12B':>12s} {'Llama-8B':>12s} {'Gemma2-9B':>12s}")
print(f"{'-'*83}")
print(f"{'Model size':<35s} {'3B':>12s} {'12B':>12s} {'8B':>12s} {'9B':>12s}")
print(f"{'Architecture':<35s} {'MHA':>12s} {'SWA+GQA':>12s} {'GQA':>12s} {'Slide+Local':>12s}")
print(f"{'Training lab':<35s} {'Alibaba':>12s} {'Mistral AI':>12s} {'Meta':>12s} {'Google':>12s}")
...
print(f"{'Length-only baseline':<35s} {'51.7%':>12s} {'59.8%':>12s} {'55.2%':>12s} {f'{length_test_acc:.1%}':>12s}")
...
if test_acc > 0.8 and p_value < 0.05:
    ...
    print("   The deception signal exists in Qwen (MHA), Mistral (SWA+GQA), Llama (GQA), AND Gemma-2 (Slide+Local).")
    print("   Four different architectures, four different labs (Alibaba, Mistral AI, Meta, Google).")
