"""
=============================================================
DeceptionProbe — 500-Scenario Experiment on Mistral-Large-Instruct-2411
=============================================================
CROSS-ARCHITECTURE GENERALIZATION TEST

Previous experiment: Qwen2.5-3B-Instruct (Alibaba) → 93.7% accuracy
This experiment:     Mistral-Large-Instruct-2411 (Mistral AI) → ???

Purpose: Prove that the deception signal is UNIVERSAL across
         COMPLETELY DIFFERENT model architectures.
         Qwen uses standard Multi-Head Attention.
         Mistral uses Sliding Window Attention + GQA.
         If both show the signal, it is architecture-independent.

Model: mistralai/Mistral-Large-Instruct-2411 (123B, 4-bit quantized)
GPU:   A100 (80GB VRAM) via Colab Pro
Layer: 44 (middle of 88 layers)
...
SAVE_DIR = Path("/content/results_mistral123b")
SAVE_DIR.mkdir(parents=True, exist_ok=True)

MODEL_NAME = "mistralai/Mistral-Large-Instruct-2411"
TARGET_LAYER = 44  # Middle of 88 layers
TRAIN_RATIO = 0.8
TRUNCATION_TOKENS = 20
SEED = 42
MAX_SEQ_LEN = 256  # Shorter to save VRAM on 123B
MAX_NEW_TOKENS = 100
np.random.seed(SEED)
...
# ============================================================
# LOAD MODEL — Mistral-Large-Instruct-2411 (4-bit quantized)
# With automatic fallback to Mistral-7B-Instruct if OOM
# ============================================================
...
try:
    ...
    SAVE_DIR = Path("/content/results_mistral7b")
    SAVE_DIR.mkdir(parents=True, exist_ok=True)
...
RESPONSES_FILE = SAVE_DIR / "generated_responses_mistral.json"
...
final_results = {
    "experiment": "DeceptionProbe — Mistral-Large-Instruct-2411 (Cross-Architecture Generalization)",
    "model": MODEL_NAME,
    ...
    "comparison_with_qwen3b": {
        ...
        "mistral_test_accuracy": float(test_acc),
        "mistral_cv_accuracy": float(cv_accuracy),
        "mistral_length_only_baseline": float(length_test_acc),
        "mistral_truncation_test": float(trunc_test_acc),
    },
}

with open(SAVE_DIR / "results_mistral.json", "w") as f:
...
print(f"\n{'='*60}")
print(f"FINAL SUMMARY — {MODEL_NAME} vs QWEN2.5-3B")
print(f"{'='*60}")
print(f"")
print(f"{'Metric':<40s} {'Qwen-3B':>10s} {'Mistral':>10s}")
print(f"{'-'*60}")
print(f"{'Model size':<40s} {'3B':>10s} {MODEL_NAME.split('/')[-1]:>10s}")
print(f"{'Architecture':<40s} {'Qwen':>10s} {'Mistral':>10s}")
...
if test_acc > 0.8 and p_value < 0.05:
    ...
    print(f"   The deception signal exists in BOTH Qwen and Mistral architectures.")
    print("   Different attention mechanisms (MHA vs SWA+GQA) both encode deception.")
    print("   This is strong evidence that the signal is a universal property of LLMs.")
elif test_acc > 0.7:
    print(f"\n⚠ PARTIAL GENERALIZATION: Signal detected in {MODEL_NAME} but weaker than Qwen.")
else:
