"""
=============================================================
DeceptionProbe — 500-Scenario Experiment (Layer 10)
=============================================================
...
LENGTH CONFOUND CONTROLS (5 methods):
  1. Strict Length Prompting ("exactly 2 sentences")
  2. Rejection Sampling (match length distributions)
  3. Truncation Test (first 20 tokens only)
  4. Residual Regression (remove length variance)
  5. Length-Only Baseline (prove length alone fails)

Two-phase design:
  Phase 1: Generate lying + honest responses (saved to JSON)
  Phase 2: Extract hidden states, train probe, run all controls

Expected runtime on T4 GPU: ~50-60 minutes
=============================================================
"""
...
SAVE_DIR = Path("/content/results_500")
...
RESPONSES_FILE = SAVE_DIR / "generated_responses_500.json"

def generate_response(context, question, mode, max_tokens=150):
...
# ============================================================
# PHASE 2: EXTRACT HIDDEN STATES — LAYER 10
# ============================================================
...
median_length = np.median(lengths_all)
short_mask = lengths_all <= median_length
long_mask = lengths_all > median_length
...
final_results = {
    "experiment": "DeceptionProbe 500-Scenario Experiment (Layer 10) — with Length Controls",
    "model": MODEL_NAME,
...
with open(SAVE_DIR / "results_500_scenarios.json", "w") as f:
...
print(f"\n{'='*60}")
print("FINAL SUMMARY — 500 SCENARIO EXPERIMENT")
print(f"{'='*60}")
print(f"Model: {MODEL_NAME}")
print(f"Method: Generation-based with length control")
print(f"Layer: {TARGET_LAYER}")
print(f"Scenarios: {len(generated_data)} ({TRAIN_SIZE} train / {TEST_SIZE} test)")
print(f"Samples: {len(generated_data)*2} ({TRAIN_SIZE*2} train / {TEST_SIZE*2} test)")
...
print(f"")
print(f"=== LENGTH CONTROLS ===")
print(f"1. Length-score correlation:  r={corr:.3f} (p={corr_p:.4f})")
print(f"2. Truncation test ({TRUNCATION_TOKENS} tokens): {trunc_test_acc:.3f}")
print(f"3. Residualized probe:       {resid_test_acc:.3f}")
print(f"4. Length-only baseline:     {length_test_acc:.3f}")
print(f"5. Short/Long subsets:       {short_acc:.3f} / {long_acc:.3f}")
print(f"")
print(f"=== WHITE LIES ===")
print(f"White lie detection: {white_lie_correct/white_lie_total:.1%}")
print(f"White lie score:     {np.mean(white_lie_scores):.3f}")
print(f"Serious lie score:   {np.mean(serious_lie_scores):.3f}")
print(f"{'='*60}")
...
if resid_test_acc > 0.7 and trunc_test_acc > 0.7:
    print("✅ LENGTH CONTROLS PASS: Deception signal is independent of response length!")
elif resid_test_acc > 0.6 or trunc_test_acc > 0.6:
    print("⚠ LENGTH CONTROLS PARTIAL: Some signal persists beyond length, but not fully conclusive.")
else:
    print("❌ LENGTH CONTROLS FAIL: Signal may be confounded with response length.")
